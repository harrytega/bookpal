...
func testBookToManyLists(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	_, err = tx.Exec("insert into \"book_list\" (\"book_id\", \"list_id\") values ($1, $2)", a.BookID, b.ListID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"book_list\" (\"book_id\", \"list_id\") values ($1, $2)", a.BookID, c.ListID)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := BookSlice{&a}
	if err = a.L.LoadLists(ctx, tx, false, (*[]*Book)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testBookToManyAddOpLists(t *testing.T) {
	var err error
...
func testBookToManySetOpLists(t *testing.T) {
	var err error
	...
	err = a.SetLists(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetLists(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
...
func testBookToManyRemoveOpLists(t *testing.T) {
	var err error
	...
	err = a.AddLists(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveLists(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
...
func testBookToOneUserUsingUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadUser(ctx, tx, false, (*[]*Book)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.User == nil {
...
func testBookToOneSetOpUserUsingUser(t *testing.T) {
	var err error
...
var (
	bookDBTypes = map[string]string{`BookID`: `uuid`, `Title`: `text`, `Author`: `text`, `Publisher`: `text`, `BookDescription`: `text`, `Genre`: `text`, `Pages`: `integer`, `Rating`: `integer`, `ImageLink`: `text`, `UserNotes`: `text`, `UserID`: `uuid`}
	_           = bytes.MinRead
)
